...
const PokemonCardWrapper = () => {
  ...
  useEffect(() => {
    const fetchPokemonData = async () => {
      try {
        ...
        const userActions = await axios.get(`https://ieee-hackathon-pokedex-backend.vercel.app/getInfo/${id}`, {
          headers: {
            Authorization: token,
  ...
  const handleCommentSubmit = async () => {
    if (!hasCommented && comments.trim()) {
      ...
      if (token) {
        const commentResponse = await axios.post('https://ieee-hackathon-pokedex-backend.vercel.app/commentPokemon', {
  ...
  const handleRateSubmit = async (star) => {
    ...
    if (token) {
      const rateResponse = await axios.post('https://ieee-hackathon-pokedex-backend.vercel.app/ratePokemon', {
        pokemon_id: pokemonId,
        rating: star
      }, {
        headers: {
  ...
  const handleToggleFavorite = async () => {
    ...
    if (token) {
      if (isFavorite) {
        await axios.delete('https://ieee-hackathon-pokedex-backend.vercel.app/removeFavouritePokemon', {
          headers: {
            Authorization: token,
            'Content-Type': 'application/json'
          },
        ...
        const favResponse = await axios.post('https://ieee-hackathon-pokedex-backend.vercel.app/addFavouritePokemon', {
          pokemon_id: pokemonId,
